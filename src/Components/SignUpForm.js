...
import { addUser } from "../state/reducers/signupReducers";
import { showToast, nullifyToast } from "../state/reducers/toastReducer";
//
export default function SignUpForm() {
  //
  const userdata = useSelector((state) => state.registeredUsers.userArray);
  const success = useSelector((state) => state.registeredUsers.success);
  const dispatch = useDispatch();
  //
  // useEffect(
  //   (e) => {
  //     console.log("userdata is changed.", success);
  //     //
  //   },
  //   [success]
  // );
  ...
  let passValidate = (userInput) => {
    ...
    ////////////////////////////////////
    if (test5 === false) {
      dispatch(showToast("Password should contain special character."));
    }

    if (test4 === false) {
      dispatch(
        showToast("Password should contain atleast one uppercase character.")
      );
    }

    if (test3 === false) {
      dispatch(showToast("Password should contain numeral."));
    }

    if (test2 === false) {
      dispatch(
        showToast("Confirmed password doesn't match with password mentioned.")
      );
    }
    if (test1 === false) {
      dispatch(
  ...
  // useEffect(
  //   (e) => {
  //     console.log("success from effect", success);
  //   },
  //   [userdata]
  // );
  //
      ...
      <form onSubmit={register}>
        <div className="card-body">
          ...
          <div className="d-flex justify-content-center mt-4">
            ...
            <Link to="/">
              <div style={{ color: "#2080ba", fontWeight: "bold" }}>
                {" "}
                Sign-In
              </div>
            </Link>
